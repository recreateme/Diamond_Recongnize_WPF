...
| [docs/CONTRACTS.md](docs/CONTRACTS.md) | 配置与结果契约（含 `summary.csv`） |
...
| `scripts/` | `publish_wpf` / `build_deploy_wpf` / `verify_wpf`；另有 `rebuild_summary_from_stats`（由 `statistics.json` 重建 `summary.csv`） |
