...
class Detection:
    ...
    defect_class: str = ""
    defect_conf: float = 0.0          # 有效类别内 argmax 概率 (= confidence)
    max_class: str = ""               # 与 defect_class 一致（有效类别 argmax）
    max_confidence: float = 0.0       # 与 defect_conf 一致
    ...
    仅接受 known_classes（有效缺陷类别）中的类别名，避免与 YOLO cls_name 混淆。

        字段说明（与 inference_common 一致）:
          · defect_class / defect_conf — 有效类别内 argmax 预测
          · max_class / max_confidence — 与 defect_class / defect_conf 相同
        """
...
def draw_classified_detections(img: np.ndarray, dets: List[Detection]) -> np.ndarray:
    ...
    标签展示有效类别 argmax 及其概率（与 defect_class 一致）。
...
class SahiPipeline:
    ...
    def process_image(
        ...
        # ── 4. 批量分类（与缺陷检测页共用同一 InferenceEngine）──
