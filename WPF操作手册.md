...
1. **分类**：对外三类（棱边朝上 / 点朝上 / 面朝上）。现行权重为真 3 类 **EfficientNetV2-S**（Letterbox 256；2026-09-18 均衡 2:2:1 微调正式版，全量 corrections Macro-F1 ≈ 0.66）；若加载历史 5 类权重，推理仍排除「局部破损」「断钻」后取有效类最高分。详见 [python_core/docs/分类模型现状与下一步.md](python_core/docs/分类模型现状与下一步.md)。
...
- 输出选项（均默认不勾，勾选写入 `app_config.json` 的 `ui_*`）：「保存可视化结果」「保存裁剪切片 crop/」「输出位置文件」「计算均匀度」（需完整模式且已勾位置文件，否则灰显）。
